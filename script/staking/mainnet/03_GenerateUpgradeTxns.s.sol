...
contract GenerateUpgradeTxns is Script, Utils {
    ...
    function _generateUpgradeTransactions(DeployedContracts memory contracts) internal {
        ...
        bytes memory upgradeCalldata = abi.encodeWithSignature(
            ...
            contracts.lrtSquaredCoreImpl,
            ""
        );
        vm.serializeString(tx1, "data", vm.toString(upgradeCalldata));
        string memory tx1Final = vm.serializeString(tx1, "description", "Upgrade LRTSquared to new Core implementation with staking support");
        ...
        bytes memory setAdminCalldata = abi.encodeWithSignature(
            "setAdminImpl(address)",
            contracts.lrtSquaredAdminImpl
        );
        vm.serializeString(tx2, "data", vm.toString(setAdminCalldata));
        string memory tx2Final = vm.serializeString(tx2, "description", "Set new Admin implementation with strategy management functions");
    ...
    function _generateConfigureTransactions(DeployedContracts memory contracts) internal {
        ...
        bytes memory ethfiConfigCalldata = abi.encodeWithSignature(
            ...
            ILRTSquared.StrategyConfig({
                strategyAdapter: contracts.sEthFiStrategy,
                maxSlippageInBps: 1
            })
        );
        vm.serializeString(tx1, "data", vm.toString(ethfiConfigCalldata));
        string memory tx1Final = vm.serializeString(tx1, "description", "Configure ETHFI token strategy for staking to sETHFI");
        ...
        bytes memory eigenConfigCalldata = abi.encodeWithSignature(
            ...
            ILRTSquared.StrategyConfig({
                strategyAdapter: contracts.eEigenStrategy,
                maxSlippageInBps: 1
            })
        );
        vm.serializeString(tx2, "data", vm.toString(eigenConfigCalldata));
        string memory tx2Final = vm.serializeString(tx2, "description", "Configure EIGEN token strategy for staking to eEIGEN");
    ...
    function _generateMigrateTransactions() internal {
        ...
        bytes memory migrateCalldata = abi.encodeWithSignature(
            "migrateTokenTypes(address[],uint8[])",
            tokens,
            types
        );
        vm.serializeString(tx1, "data", vm.toString(migrateCalldata));
        string memory tx1Final = vm.serializeString(tx1, "description", "Migrate token types: Native (EIGEN,ETHFI,WETH,SWELL) and Staked (sETHFI,eEIGEN)");
