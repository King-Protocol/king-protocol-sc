...
library DeploymentLibrary {
    ...
    function deployStrategies() internal returns (address sEthFiStrategy, address eEigenStrategy) {
        ...
        sEthFiStrategy =
            address(new SEthFiStrategy(DeploymentConstants.LRT_SQUARED_PROXY, DeploymentConstants.PRICE_PROVIDER));
        emit DeploymentStep("SEthFiStrategy", sEthFiStrategy);

        // Deploy new EEigenStrategy with vault's price provider
        eEigenStrategy =
            address(new EEigenStrategy(DeploymentConstants.LRT_SQUARED_PROXY, DeploymentConstants.PRICE_PROVIDER));
    ...
    function configureStrategies(address sEthFiStrategy, address eEigenStrategy) internal {
        ...
        lrtSquared.setTokenStrategyConfig(
            DeploymentConstants.ETHFI,
            ILRTSquared.StrategyConfig({strategyAdapter: sEthFiStrategy, maxSlippageInBps: 1})
        );

        // Configure strategy for EIGEN token
        lrtSquared.setTokenStrategyConfig(
            DeploymentConstants.EIGEN,
            ILRTSquared.StrategyConfig({strategyAdapter: eEigenStrategy, maxSlippageInBps: 1})
        );
    }
    ...
    function verifyDeployment(Vm vm, DeploymentResult memory deployment) internal view {
        ...
        ILRTSquared.StrategyConfig memory ethfiStrategyConfig =
            lrtSquared.tokenStrategyConfig(DeploymentConstants.ETHFI);
        ILRTSquared.StrategyConfig memory eigenStrategyConfig =
            lrtSquared.tokenStrategyConfig(DeploymentConstants.EIGEN);
