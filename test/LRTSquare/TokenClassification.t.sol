...
contract TokenClassificationTest is LRTSquaredTestSetup {
    ...
    function setUp() public override {
        ...
        mockStrategy = new MockStrategy(address(lrtSquared), address(priceProvider), address(stakedToken), address(nativeToken));
    ...
    function test_RegisterTokenWithType() public {
        ...
        assertEq(uint(info.tokenType), uint(ILRTSquared.TokenType.Native), "Token type should be Native");
    ...
    function test_RegisterMultipleTokenTypes() public {
        ...
        assertEq(uint(nativeInfo.tokenType), uint(ILRTSquared.TokenType.Native));
        assertEq(uint(stakedInfo.tokenType), uint(ILRTSquared.TokenType.Native)); // Must be Native without strategy
        assertEq(uint(rewardInfo.tokenType), uint(ILRTSquared.TokenType.Native));
    }
    ...
    function test_SetTokenType() public {
        ...
        lrtSquared.setTokenStrategyConfig(
            address(nativeToken),
            ILRTSquared.StrategyConfig({
                strategyAdapter: address(mockStrategy),
                maxSlippageInBps: 100
            })
        );
        ...
        assertEq(uint(info.tokenType), uint(ILRTSquared.TokenType.Staked), "Type should be updated to Staked");
    }
    ...
    function test_MigrateTokenTypes() public {
        ...
        assertEq(uint(nativeInfo.tokenType), uint(ILRTSquared.TokenType.Native));
        assertEq(uint(stakedInfo.tokenType), uint(ILRTSquared.TokenType.Staked)); // Properly set to Staked
        assertEq(uint(rewardInfo.tokenType), uint(ILRTSquared.TokenType.Native));
    }
    ...
    function test_TokenTypePersistsThroughOperations() public {
        ...
        ILRTSquared.TokenInfo memory info = lrtSquared.tokenInfos(address(stakedToken));
        assertEq(uint(info.tokenType), uint(ILRTSquared.TokenType.Staked), "Type should remain Staked");
    ...
    function test_DefaultTokenTypeForLegacyTokens() public {
        ...
        assertEq(uint(defaultType), uint(ILRTSquared.TokenType.Native), "Default should be Native");
    ...
    function test_CanRegisterTokenAsStakedWithStrategy() public {
        ...
        ILRTSquared.TokenInfo memory info = lrtSquared.tokenInfos(address(stakedToken));
        assertEq(uint(info.tokenType), uint(ILRTSquared.TokenType.Staked), "Token should be Staked");
    ...
    function test_BackwardCompatibleRegisterToken() public {
        ...
        assertTrue(info.registered, "Token should be registered");
        assertEq(uint(info.tokenType), uint(ILRTSquared.TokenType.Native), "Token type should default to Native");
