...
contract LRTSquaredRedeemTest is LRTSquaredTestSetup {
    ...
    function test_RedeemEmitsCorrectEvent() public {
        ...
        (address[] memory previewTokens, uint256[] memory previewAmounts, uint256 previewFee) =
            lrtSquared.previewRedeem(sharesAlloted);
    ...
    function test_RedeemWithDifferentTokenDecimals() public {
        ...
        assertApproxEqAbs(
            IERC20(_tokens[0]).balanceOf(alice), expectedToken0, 10, "Token0 (18 decimals) amount incorrect"
        );
        assertApproxEqAbs(
            IERC20(_tokens[1]).balanceOf(alice), expectedToken1, 10, "Token1 (6 decimals) amount incorrect"
        );
        assertApproxEqAbs(
            IERC20(_tokens[2]).balanceOf(alice), expectedToken2, 10, "Token2 (8 decimals) amount incorrect"
        );
    }
