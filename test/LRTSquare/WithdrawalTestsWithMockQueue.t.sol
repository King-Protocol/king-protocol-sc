...
contract MockAtomicQueue {
    ...
    function updateAtomicRequest(address offer, address want, IAtomicQueue.AtomicRequest calldata atomicRequest)
        external
    {
        lastOffer = offer;
        lastWant = want;
...
contract TestableBoringStrategy is BoringVaultStrategyBase {
    ...
    // Override _calculateWithdrawalPrice to avoid external calls
    function _calculateWithdrawalPrice() internal pure override returns (uint256) {
...
contract WithdrawalTestsWithMockQueue is Test {
    ...
    function setUp() public {
        ...
        strategy = new TestableBoringStrategy(
            vault, priceProvider, boringVault, address(mockQueue), address(stakedToken), address(nativeToken)
        );
