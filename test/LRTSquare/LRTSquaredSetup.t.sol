...
contract LRTSquaredTestSetup is Utils {
    ...
    function _registerToken(address token, uint256 tokenMaxPercentage, bytes memory revertData) internal {
        ...
        bytes memory data = abi.encodeWithSelector(
            bytes4(keccak256("registerToken(address,uint64)")),
            token,
            tokenMaxPercentage
        );
    ...
    function _registerTokenWithType(address token, uint256 tokenMaxPercentage, ILRTSquared.TokenType tokenType, bytes memory revertData) internal {
        string memory description = string(
            abi.encodePacked(
                "Proposal: Register token with type: ",
                vm.toString(token),
                " at time ",
                vm.toString(block.timestamp)
            )
        ...
        bytes memory data = abi.encodeWithSelector(
            bytes4(keccak256("registerToken(address,uint64,uint8)")),
            token,
            tokenMaxPercentage,
            tokenType
        );
