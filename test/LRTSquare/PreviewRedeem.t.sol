...
contract PreviewRedeemTest is LRTSquaredTestSetup {
    ...
    function setUp() public override {
        ...
        lrtSquared.setFee(ILRTSquared.Fee({
            treasury: treasury,
            depositFeeInBps: 0,
            redeemFeeInBps: 0
        }));
    ...
    function test_03_NativeOnly_OnlyNativeInVault() public {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            if (tokens[i] == address(ethfi)) ethfiAmount = amounts[i];
                ...
                assertEq(uint8(tokenInfo.tokenType), uint8(ILRTSquared.TokenType.Native), 
                    "Should only receive native tokens");
            }
        }
    ...
    function test_06_InsufficientNative_UseStaked() public {
        ...
        // Verify total value
        uint256 totalValue = 
            ethfiAmount * ETHFI_PRICE / 1e18 +
            eigenAmount * EIGEN_PRICE / 1e18 +
            swellAmount * SWELL_PRICE / 1e18 +
            sethfiAmount * 1e18 / 1e18 +
            eeigenAmount * 1e18 / 1e18;
    ...
    function test_08_LockedTokens_RequiresStakedRebalancing() public {
        ...
        // Verify total value
        uint256 totalValue = 
            ethfiAmount * ETHFI_PRICE / 1e18 +    // 2.5 ETH
