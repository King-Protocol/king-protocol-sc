...
interface IAtomicQueue {
    ...
    function getUserAtomicRequest(address user, address offer, address want)
        external
        view
...
abstract contract BoringVaultStrategyBase is BaseStrategy {
    ...
    function returnToken() public view virtual override returns (address);
    ...
    function initiateWithdrawal(uint256 shareAmount) external virtual override {
        ...
        // Get return token address
        address returnTokenAddr = returnToken();
        ...
        IAtomicQueue.AtomicRequest memory existingRequest =
            IAtomicQueue(queue).getUserAtomicRequest(address(this), returnTokenAddr, token());
        ...
        uint256 atomicPrice = _calculateWithdrawalPrice();
    ...
    function cancelWithdrawal() external virtual {
        ...
        IAtomicQueue.AtomicRequest memory cancelRequest =
    ...
    function _deposit(address, uint256 amount, uint256 maxSlippageInBps) internal virtual override {
        ...
        address stakedToken = returnToken();
        ...
        uint256 sharesReceived =
            ITellerWithMultiAssetSupport(boringVault).deposit(ERC20(nativeToken), amount, minShares);
    ...
    /**
     * @notice Calculate withdrawal price with discount to incentivize solvers
     * @dev Applies maxSlippageInBps as a discount to the market price ratio
     * to make withdrawal orders attractive for solvers to fill after cooldown
     * @return Discounted price ratio (native tokens per staked token)
     */
    function _calculateWithdrawalPrice() internal view virtual returns (uint256) {
        uint256 stakedTokenPrice = priceProvider.getPriceInEth(returnToken());
